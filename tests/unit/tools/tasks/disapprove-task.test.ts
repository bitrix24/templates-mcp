...
describe('bitrix24_disapprove_task', () => {
  ...
  it('calls tasks.task.disapprove and returns the disapproved-task summary', async () => {
    callMethod.mockResolvedValue({
      getData: () => ({ result: { task: { id: 8017, title: 'x', status: '2', responsibleId: '547' } } }),
    })

    const result = await tool.handler({ taskId: 8017 })

    expect(callMethod).toHaveBeenCalledWith('tasks.task.disapprove', { taskId: 8017 })
