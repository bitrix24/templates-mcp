...
describe('bitrix24_start_task', () => {
  ...
  it('calls actions.v3.call.make with tasks.task.start and returns the started-task summary', async () => {
    fake.v3Call.mockResolvedValue(fakeOk({ task: { id: 8017, title: 'do it', status: '3', responsibleId: '503' } }))

    const result = await tool.handler({ taskId: 8017 })

    expect(fake.v3Call).toHaveBeenCalledWith({ method: 'tasks.task.start', params: { taskId: 8017 } })
