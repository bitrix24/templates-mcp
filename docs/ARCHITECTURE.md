...
| Tool registry | Nuxt file-glob (HTTP) / `mcp-stdio/tools.ts` (stdio, hand-maintained) | parity is CI-enforced — see hot spots |
...
2. **Tenant-dispatcher invariant.** Tool handlers MUST resolve their client via `useBitrix24Tenant()` from `server/utils/bitrix24-tenant.ts`, never `useBitrix24()` directly. Direct webhook calls bypass the OAuth dispatcher and silently break multi-tenant mode — there is no CI guard for this (the parity test checks registration, not handler contents). See [`OAUTH-DESIGN.md` §6](./OAUTH-DESIGN.md#6-mcp-bearer--tenant-token-coupling) and `docs/ADDING-TOOLS.md`.
