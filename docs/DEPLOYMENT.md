...
- **JSON-RPC `initialize` round-trip** → `POST /mcp` with a real `initialize` payload and the proper `Accept: application/json, text/event-stream` header; the response must be a valid JSON-RPC envelope (`jsonrpc: "2.0"`, matching `id`, `result.protocolVersion` populated) AND `result.serverInfo.name === "bx24-template-mcp"` AND `result.capabilities.tools` advertised. A follow-up `tools/list` (sent without a `Mcp-Session-Id` — `@nuxtjs/mcp-toolkit`'s node provider defaults to **stateless mode**, meaning each `/mcp` request is independently authenticated and the SDK neither issues nor requires a session header) must include `b24_user_me` in the catalogue. This is the assertion that pins the `@nuxtjs/mcp-toolkit` dispatcher; without it a middleware refactor whose error lands in Nitro's uncaught-error handler (becoming a `500` instead of a JSON-RPC error envelope) ships green under the four checks above. **Requires `jq`** — if `jq` is not on PATH this assertion is **skipped** with a notice (the JSON-RPC predicates are too brittle without real parsing).
