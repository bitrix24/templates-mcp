...
- **Never call `useBitrix24()` directly from a tool handler** — it pins the tool to the webhook path forever and silently breaks multi-tenant mode. The CI naming-convention test won't catch this; the breakage only shows up when an operator flips the OAuth flag on. Always go through `useBitrix24Tenant()`. The dispatcher (`server/utils/bitrix24-tenant.ts`) does the right thing under both flag states: with `NUXT_BITRIX24_OAUTH_ENABLED=false` (the production default) it returns the same webhook-backed singleton a direct `useBitrix24()` call would; with the flag on, it returns a per-tenant `B24OAuth` resolved from the request's ALS context, so every REST call runs under the end user's identity. Design lives in [`OAUTH-DESIGN.md` §6](./OAUTH-DESIGN.md#6-mcp-bearer--tenant-token-coupling).
