...
- **Workflow security audit (`zizmor`) — ENFORCING** (issue #178). The `zizmor` CI job blocks the build on any finding in `.github/workflows/**` (template injection, artifact/cache poisoning, dangerous triggers, excessive permissions). It runs a **pinned** binary (`version: 1.25.2`) in **offline** mode so the gate is reproducible against a local `zizmor --offline .github/workflows/` and isn't perturbed by live advisory-DB changes. Accepted findings are annotated inline with `# zizmor: ignore[<audit>]` + a justification (see `deploy.yml`). Bump the pinned version deliberately and re-triage. `actionlint` runs alongside but stays **advisory** (`continue-on-error: true`) until its own first-run findings are triaged (#179) — security findings block, syntax findings warn.
