...
- **OAuth surface inertness (webhook-only deploys).** When `NUXT_BITRIX24_OAUTH_ENABLED=false` (the default), every `/api/oauth/*` route is gated by **three independent flag-checks** before any side-effect: (1) the Nitro plugin `server/plugins/oauth-schema.ts` early-returns at boot and never installs the SQLite schema; (2) the h3 middleware (`server/middleware/oauth-rate-limit.ts`, `mcp-auth.ts`) refuses requests; (3) `server/utils/token-store.ts:useTokenStore()` **throws** rather than silently creating the DB — the message reads "this is a wiring bug" so a future regression cannot silently land an empty `oauth.sqlite`. Each `/api/oauth/install` / `/callback` / `/_health` route checks the flag BEFORE any `useTokenStore()` call (verified in code: 121, 37, and 62 lines of distance respectively), so the worst webhook-only operators can see is a `503 FLAG-OFF` response — never a touched filesystem, never an open database handle. Defence-in-depth recipe for webhook-only deploys: deny `/api/oauth/*` at the reverse-proxy layer so hostile probes don't even reach the Node.js process. The repo ships a ready snippet at [`nginx/vhost.d/oauth-deny.conf.example`](../nginx/vhost.d/oauth-deny.conf.example) and per-stack installation recipes (plain nginx / our nginx-proxy stack / Caddy) in [`REVERSE-PROXY.md` → "Hardening: deny /api/oauth/ on webhook-only deploys"](./REVERSE-PROXY.md#hardening-deny-apioauth-on-webhook-only-deploys). Remove the proxy block before flipping the flag on.
