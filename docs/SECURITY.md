...
- DoS mitigation beyond Docker resource limits — **except** the per-IP rate limit on `/api/oauth/install` (issue #221), which caps the `oauth_state`-flood vector at the application layer. General load-shedding / L7 DDoS protection is still the reverse proxy's job.
