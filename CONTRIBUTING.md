...
> **CI authoring rule (closes #171):** Never interpolate `${{ … }}` directly inside a `run:` block (or an SSH-action `script:` block) — GitHub substitutes the expression into the script source before the shell parses it, so a malicious PR title or comment can execute commands on the runner. Bind the value through `env:` and reference it with `$VAR` in the script body (see the `commitlint` and `deploy` jobs for the canonical shapes — `deploy` also forwards env-vars into the SSH session via appleboy/ssh-action's `envs:` parameter). The same pattern applies to `vars.*` and `secrets.*` — even repo-controlled values benefit from env-binding for defence-in-depth and readability. The `lint` job has a regex guard that fails the build if a user-controllable event field (title / body / message) is reintroduced inline; `vars.*` regressions need to be caught at review time until an `actionlint`/`zizmor` gate lands.
