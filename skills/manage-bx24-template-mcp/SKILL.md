...
9. **Destructive operations on cascade-structured data need an explicit confirm flag.** When a Bitrix24 method silently destroys more than the agent meant to — e.g. deleting a checklist heading wipes every child item, or deleting a workgroup wipes every task in it — add a `confirm<Action>: boolean` field to the Zod schema and refuse with a typed `Bitrix24ToolError` (code `*_NEEDS_CONFIRM`) unless the agent set it to `true`. The error message MUST name the cascade target and tell the agent how to re-call. The precedent is `bitrix24_delete_checklist_item` + `confirmDeleteHeading` (see `server/utils/checklist.ts` `assertNotHeading` / `assertBatchNoHeadings`); a single pre-flight `callV2` (`task.checklistitem.getlist`) gates both single-id and batch flows. The pre-flight cost is acceptable for destructive ops; for batches use ONE shared pre-flight rather than N individual `get` calls. This rule **stacks on top of** Ground Rule #10 — cascade deletes require BOTH `confirmDelete: true` (universal) AND `confirm<Cascade>: true` (cascade-specific).

10. **Every delete tool requires an explicit `confirmDelete: true` from the agent.** Deletion is irreversible from the MCP's side — Bitrix24 does not surface a "trash" or "undo" for the entities we expose. To prevent LLM mis-interpretation ("посмотри запись 5" → tool called `delete_*` instead of `list_*`), every `bitrix24_delete_*` tool MUST add a `confirmDelete: boolean` field to its Zod schema and refuse with `Bitrix24ToolError` code `DELETE_NEEDS_CONFIRM` unless the agent set it to `true`. The error message MUST name the target(s) so the agent shows the operator what they're agreeing to. Applies to BOTH single and batch — the confirm is per-call, not per-id (one confirm authorises the whole batch). The shared schema fragment lives at `server/utils/define-action-tool.ts` (`confirmDeleteSchema()`); use it directly to keep wording uniform across delete tools. Precedent: `bitrix24_delete_elapsed_time` (PR-B). Existing delete tools that pre-date this rule (`bitrix24_delete_task_result`, `bitrix24_delete_checklist_item`) need retrofit — tracked separately.
