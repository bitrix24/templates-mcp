...
4. Name pattern: `b24_<domain>(_<entity>)*_<action>` for Bitrix24 tools (e.g. `b24_task_create`, `b24_task_checklist_item_add`, `b24_user_me`); `bx24mcp_<verb>` for meta-tools. **Action is always the trailing token.** Entity is singular by default; for `_list` tools it is **plural** (`b24_tasks_list`, `b24_task_results_list`). The `b24_*` pattern is enforced by `tests/unit/mcp-stdio/tool-naming-convention.test.ts` — adding a name that doesn't match fails CI.
