...
7. **Prefer REST API v3 where it exists; treat the v2 endpoints below as canonical, not "fallback".** Use v3 (apidocs URLs containing `rest-v3/`) for everything that has it. The following families are **v2-canonical and not coming to v3** — don't waste time hunting for a v3 equivalent, just `callV2`: `user.*`, `task.commentitem.*`, `task.checklistitem.*`, `task.elapseditem.*`, `task.dependence.*` (modify), `task.item.getdependson` (read; deprecated upstream but the only documented path for predecessor lists as of 2026-05). Bitrix24's v3 `tasks.template.checklist.*` is for task templates only; it does NOT replace `task.checklistitem.*` for actual tasks. For any other v2 method (e.g. `task.*` legacy CRUD), prefer v3 and document the v2 fallback in the tool's docstring with a link to the apidocs page.
