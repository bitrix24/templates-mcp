...
- **DXT + OAuth via Bitrix24 OOB code-paste flow (#207).** The local-stdio `.dxt` bundle can now act under each user's own Bitrix24 identity instead of a shared webhook service account. Operators leave the webhook URL empty and set the new `bitrix24_portal_host` `user_config` field; on first launch the extension log prints `https://<portal>/oauth/authorize/?client_id=<baked>&state=<random>` — no `redirect_uri` (the [official Bitrix24 OOB protocol](https://apidocs.bitrix24.ru/api-reference/oauth/index.html) shows the code inline on the consent page for installations without a fixed callback). The agent then calls `bx24mcp_oauth_paste_code` (new stdio-only tool) with the short code, and the bundle exchanges it for a per-user access/refresh token pair. Tokens land in `<user-data>/bx24-template-mcp/oauth.json` (file mode 0o600, atomic write via tmp + rename); the SDK silently refreshes the access token on 401 via a custom refresh handler that mirrors `server/utils/bitrix24-oauth.ts` and persists new tokens through the same path. Bundle-side audit log at `<user-data>/audit.log` (JSONL, same taxonomy as the HTTP server's `audit-log.ts`: `oauth.upsert.exchange` / `oauth.upsert.refresh` / `oauth.fail.invalid-grant` / `oauth.fail.transient`). When the refresh token is revoked on the portal side the store gets a sticky `refreshInvalid: true` flag and tools return a friendly "re-onboarding required" message until `bx24mcp_oauth_paste_code` is re-run. `BITRIX24_DXT_OAUTH_CLIENT_ID` / `_SECRET` are baked at build time via esbuild `define` (upstream CI pulls them from repo secrets; forks set them in env before `pnpm build:dxt`). A bundle built without either secret silently runs webhook-only — the runtime check in `mcp-stdio/auth-mode.ts` gates the OAuth path on a non-empty client id, so empty-build = same behaviour as v0.2.0. Localised install guides in `mcp-stdio/INSTALL.{ru,pt-BR}.md` gained a new OAuth section.
